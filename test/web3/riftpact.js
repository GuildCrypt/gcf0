...
describe('RiftPact', () => {
  before(async () => {
    ...
    OathForge = await resolver.require('OathForge')
    RiftPact = await resolver.require('RiftPact')
    ERC20 = await resolver.require('ERC20')
    //
    ...
    oathForge = await OathForge.new('OathForge', 'OAT',{ from: accounts[0] })
    erc20 = await ERC20.new({ from: accounts[0] })

    // Create a token in the oathforge contract before deploying the riftpact
    await oathForge.mint(accounts[1], 'https://example.com', 1e3, { from: accounts[0] })
    ...
    riftPact = await RiftPact.new(oathForge.address, tokenId, totalSupply, erc20.address, auctionAllowedAt, waitAfterLastBidToCloseAuction, minIncrementPerBid, { from: accounts[0] })
  })
  it('Should deploy a new riftpact successfully', async () => {
    const tokenId = parseInt(await oathForge.nextTokenId()) - 1
    const totalSupply = 1e6 // 6 million erc20 tokens
    const erc20 = await ERC20.new({ from: accounts[0] })
    ...
    const riftPactTwo = await RiftPact.new(oathForge.address, tokenId, totalSupply, erc20.address, auctionAllowedAt, waitAfterLastBidToCloseAuction, minIncrementPerBid, { from: accounts[0] })
  ...
  it('Should start an auction successfully when the allowed time is reached', async () => {
    ...
    await riftPact.startAuction({ from: accounts[1] })
  ...
  it('Should bid on a started auction successfully', async () => {
    ...
    // Start the auction
    await asyncSetTimeout(2) // Wait until the auction is allowed
    await riftPact.startAuction({ from: accounts[0] })
    const timeAuctionStarted = parseInt(await riftPact.auctionStartedAt())
  ...
  it('Should send the payout to the latest bidder after the auction is ended', async () => {
    ...
    // Send payout
    await riftPact.payout({ from: accounts[0] })
